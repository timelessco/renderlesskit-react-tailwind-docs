...
const Layout = ({ filename, config: _config, pageMap, meta, children }) => {
  ...
  const directories = useMemo(() => cleanupAndReorder(pageMap, locale, defaultLocale), [
    pageMap,
    locale,
    defaultLocale,
  ]);
  ...
  const currentIndex = useMemo(() => flatDirectories.findIndex((dir) => dir.route === fsPath), [
    flatDirectories,
    fsPath,
  ]);
    ...
    <React.Fragment>
      ...
      >
        <nav className='fixed top-0 left-0 right-0 z-20 flex items-center h-16 px-6 bg-white border-b border-gray-200 dark:bg-dark dark:border-gray-900'>
          ...
          {config.customSearch ||
            (config.search ? config.UNSTABLE_stork ? <StorkSearch /> : <Search directories={flatDirectories} /> : null)}
          ...
          {config.repository ? (
            <a className='p-2 text-current' href={config.repository} target='_blank'>
        ...
        <ActiveAnchor>
          <div className='flex flex-1 h-full'>
            ...
            <SkipNavContent />
            {meta.full ? (
              <article className='relative w-full pt-16 overflow-x-hidden'>{children}</article>
            ) : (
